...
parameter usually diverges. An example of an order parameter is the 
magnetization in a ferromagnetic system. 
 
Second order phase transitions, such as the magnetization in Ising model, and 
spin glass transition in Edwards-Anderson model, are characterized by their 
...
\begin{equation}
  ...
  =g_c(tL^{1/\nu})
\end{equation}
therefore, $U_L$ tends towards an universal value.

So one can use various system sizes $L$, calculate $U_L$s as functions of $T$,
and find the point where the $U_L(T)$ curve cross to identify $T_c$. 
