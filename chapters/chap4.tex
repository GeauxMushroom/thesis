...
\chapter{Continuous Time Quantum Monte Carlo Solver for Strongly Correlated Materials}
\label{chap:cthyb}
...
approximation (LDA) \cite{lundqvist1983}, density functional theory produces satisfactory data that agrees well the 
...
The accuracy of density functional theory has seen gradually improve over the year, the continual
...
density functional theory. A promising direction is to combine other methods which can treat
strong correlation with the density functional theory method. A popular choice is to employ 
...
correlation on top of the single particle dispersion obtained from the density functional theory. 

%DFT, ab inito

%maybe mention other numerical methods here?

Dynamical mean field theory has been widely used on a range of strongly correlated systems. 
...
\subsection{Using Legendre Polynomials for Measurement}
