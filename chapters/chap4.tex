...
After about nine decades, the physics of non-interacting electrons are
still being investigated intensively today. The exotic physics from
...
single particle picture, its physics can be studied relatively easily by
the available numerical methods. It is in general very difficult to analysis
a many particles interacting systems by simple numerical mean.

However if one can reduce the systems into a somewhat single particle systems,
...
circumstances the interacting can be reduced into single particles system with
a modification. The non-interacting particle can be adiabatically deformed
into quasi-particle with a finite lifetime. This important prediction by Landau
can only explained few decades later when the concepts of renormalization group
...
system, its limitation is obvious. The clear problem is in the low-dimensional
cases, by now, we know this is invalid in one dimension. However, its validity
in two dimension is more questionable. As many of interesting systems, the
physics are believed to be largely dictated by the correlation in two dimensions.
Besides, that the Fermi liquid theorem cannot describe the quantum criticality,
it is obvious that the criticality involve collective behaviors. Thus an
independent particle picture is doomed to fail to describe critical point.

In a sense, the term strong correlation refers to the behavior of electrons
...
unfilled d or f orbitals. This can be understood as the orbital radius of
d and f orbitals tend to be small and so does the overlap among orbitals.
This reduces the kinetic energy of the system, and thus the interaction
becomes more important. Many interesting materials discovery which host
...
metal and inner transition metal. This is precisely because of the effective
...
to a regime almost all analytical methods will fail.
